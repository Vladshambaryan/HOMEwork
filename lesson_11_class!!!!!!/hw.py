class Book:
    ...
    def details(self):
        # информации о книге
        details = (f"Название: {self.title}, Автор: {self.author}, "
...
class SchoolBook(Book):
    def __init__(self, title, author, pages, isbn, subject, grade_level, has_exercises, reserved=False):
        # Инициализация атрибутов родительского класса
        super().__init__(title, author, pages, isbn, reserved)
        # Дополнительные атрибуты для школьных учебников
        self.subject = subject
        self.grade_level = grade_level
        self.has_exercises = has_exercises

    def details(self):
        # Форматирование информации о школьном учебнике
        details = (f"Название: {self.title}, Автор: {self.author}, страниц: {self.pages},"
                   f" предмет: {self.subject}, класс: {self.grade_level}")
        if self.reserved:
            details += ", зарезервирована"
        return details

# Создание экземпляров учебников
school_book1 = SchoolBook("Алгебра", "Иванов", 200, "2234567890","Математика", 9, True)

school_book2 = SchoolBook("История", "Петров", 250, "2234567891","История", 10, False)

school_book3 = SchoolBook("География", "Сидоров", 150, "2234567892","География", 8, True, reserved=True)

school_book4 = SchoolBook("Физика", "Кузнецов", 300, "2234567893","Физика", 11, True)

school_book5 = SchoolBook("Химия", "Васильев", 220, "2234567894","Химия", 9, True)

# Вывод информации о каждом учебнике
